...
var verifyReporter = (req, res, next) => {
...
app.get("/api/reports", async (req, res) => {
  try {
    ...
    if (sortBy === "most_upvoted") {
      ...
      const results2 = await reports.aggregate(pipeline).toArray();
      res.status(200).json({ reports: results2, totalPages, currentPage: pageNum, total });
      return;
    }
...
app.get("/api/reports/safety-stats", async (req, res) => {
  try {
    const {
      type = "districts",
      q,
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const pullFields = {};
      Object.values(arrayMap).forEach((arr) => {
